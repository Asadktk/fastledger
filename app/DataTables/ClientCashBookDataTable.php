...
class ClientCashBookDataTable extends DataTable
{
    ...
    //                 // Use window function to calculate the running balance, alias as 'Balance' instead of 'Running_Balance'
    //                 DB::raw("SUM(CASE WHEN Transaction.Paid_In_Out = 1 THEN Transaction.Amount ELSE 0 END)
    //                               OVER (ORDER BY Transaction.Transaction_Date ASC) -
    //                             $initialBalance +  SUM(CASE WHEN Transaction.Paid_In_Out = 2 THEN Transaction.Amount ELSE 0 END)
    //                               OVER (ORDER BY Transaction.Transaction_Date ASC) AS Balance"),
