    ...
    <script>
        $(document).ready(function () {
            ...
            if (fromDate || toDate || bankAccountId) {
                $.ajax({
                    url: '{{ route("office.cashbook.get_initial_balance") }}', // Correct route for fetching initial balance
                    method: 'GET',
                    data: {
                        from_date: fromDate,
                        to_date: toDate,
            ...
            $('#filter-form').on('submit', function (e) {
                ...
                // Reload the DataTable with the new parameters (correct URL formatting)
                table.ajax.url(`{{ route('office.cashbook') }}?${params.toString()}`).load(function () {
